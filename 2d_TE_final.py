# -*- coding: utf-8 -*-
"""
Created on Wed Jul 23 21:07:54 2014
...
def c_TDMA_Solver(a,b,c,d):
    ...
    code="""
        ...
        """
    inline(code,['a','b','c','d','x','gamma','n','beta'],type_converters=blitz,compiler='gcc')
    return x
...
Nx=512                         #no. of points to keep in x-direction
Nskip=1
Npts=int(Nz/Nskip)
z=linspace(0,L,Npts)
Lx=20
...
x0=0
sig=4
Ex=1*exp(-(x-x0)**2/sig**2).astype(complex)
EX[0,:]=Ex
mxm[0]=(abs(Ex).max())
