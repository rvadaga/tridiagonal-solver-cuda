   ...
   {//begin of else
	...
	if(fp == NULL)
	   ...
	   {//begin of else - else
		   ...
		   {				
			...
			switch(count)
				...
				case 4://node of type 1
					...
					if(s[0]=='r'||s[0]=='R'||s[0]=='l'||s[0]=='L'||s[0]=='c'||s[0]=='C'||s[0]=='i'||s[0]=='I'||s[0]=='v'||s[0]=='V')
			...
			if((strcmp(s, ".end")==0) || (strcmp(s, ".end\n")==0))//checks for end of circuit definition
				{
				row+=2;
		...
		Node *p=(Node *)malloc(sizeof(Node));//node created for printing linked list's nodes
	   ...
	   //of the same order that has been entered 
		fprintf(fp1, "The elements of the linked list are written in a tabular form below.\n");
		fprintf(fp1, "Name      Nodes         depname   depnodes            value\n");
		while(p!=NULL)
  			{
	   ...
	   fprintf(fp1, "\n");
		...
		for(row=row+1; ; row++)
		   {				
			...
			if(fgets(s, BUFLEN-1, fp)==NULL)//breaks when there are no more lines to read
			break;
			if((strcmp(s, ".end")==0) || (strcmp(s, ".end\n")==0))//checks for end of circuit analysis definition
				{
		...
		char **node_name=(char **)malloc(sizeof(char *)*(2*list->count));//node_name to store distinct node names
      ...
      printf("Log file \"log_file.txt\" has been created to maintain a record of the various\noperations that have been done to find the solution.\n"); 
	  	printf("In order to see the log file \"log_file.txt\" clearly, deselect the text wrapping mode.\n\n");
...
//function to add node after it is valid
void add_node(List *list, Node *node)
{
if(list->head!=NULL)//for assigning the remaining nodes
   {
	list->head->prev=node;//assigning prev ptr of list head to node being added 
	node->prev=NULL;//assigning prev of node being added to NULL because it'll be the new list head		
	node->next=list->head;//new node's next is pointing to old list head node 
	list->head=node;//new node made the lists's head
	(list->count)+=1;//incrementing list's count
   }
else//for assigning head of the list
   {
	list->head=node;//assigns head to node being added
	list->head->prev=NULL;//assigns prev ptr of list head to NULL
	(list->count)+=1;//increments list's count
   }
}
...
for(n=0, pbuf=s; ; n++)
	...
	if(n==3)
		...
		if(strcmp(word, "0")!=0)		
		node->value=extract_modifier(word, fp1);//modifying the value based on following character
		else
		node->value=0;			
		}
	pbuf=NULL;
...
if((node->value!=0 && strcmp(node->n1, "invalid")!=0 && strcmp(node->n2, "invalid")!=0) || ((*node->name=='V' || *node->name=='v') && strcmp(node->n1, "invalid")!=0 && strcmp(node->n2, "invalid")!=0) )
	{
	node->n4=node->depname=node->n3=NULL;
	add_node(list, node);
	printf("Node name: %s, nodes: %s and %s, value: %E\n\n", node->name, node->n1, node->n2, node->value);
...
//node printed after checking validity of it's members
if(node->value!=0 && node->depname!=NULL && strcmp(node->n1, "invalid")!=0 && strcmp(node->n2, "invalid")!=0)
	{
...
//node printed after checking validity of it's members
if(node->value!=0 && strcmp(node->n1, "invalid")!=0 && strcmp(node->n2, "invalid")!=0 && strcmp(node->n3, "invalid")!=0 && strcmp(node->n4, "invalid")!=0)
	{
	node->depname=NULL;
	add_node(list, node);
...
//checking if node name begin;s with N or n and if there are no special characters or not
...
if(c==0)
	{
	printf("Voltage source %s given in current controlled source %s does not exist.\n", p->depname, p->name);
	printf("Program will terminate.\n\n");
	fprintf(fp1,"Voltage source %s given in current controlled source %s does not exist.\n", p->depname, p->name);
	fprintf(fp1, "Program will terminate.\n\n");
	exit(8);
	}
}

//function to provide each node name to add it to distinct nodes array if it does not repeat
...
//search_node_name function is used to check if that node name is already present and adds it if it's not present
if(c==4 || c==5)
	{
...
while(p!=NULL)
	{	
	if(*p->name=='V' || *p->name=='v' ||*p->name=='E' || *p->name=='e' || *p->name=='H' || *p->name=='h')
	c++;
	p=p->next;
	}
return c;
...
//function to add current unknown in vector B based on the voltage source on which the element is dependent on. 
...
while(p!=NULL)
	...
	if(*(p->name)=='V' || *(p->name)=='v')
		{
		sprintf(*(c+i), "I[%d][%d]", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2));
		fprintf(fp1, "For voltage source %s, current I[%d][%d] has been added at row %d.\n", p->name, node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), i);
		i++;
		}
	...
	if(*(p->name)=='E' || *(p->name)=='e')
		{
		sprintf(*(c+i), "I[%d][%d]", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2));
		fprintf(fp1, "For VCVS %s, current I[%d][%d] has been added at row %d.\n", p->name, node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), i);
		i++;
		}
	...
	if(*(p->name)=='H' || *(p->name)=='h')
		{
		sprintf(*(c+i), "I[%d][%d]", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2));
		fprintf(fp1, "For CCVS %s, current I[%d][%d] has been added at row %d.\n", p->name, node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), i);
		i++;
		}
	p=p->next;
	}
...
//function to find a ground node which has name "0", and to return the value of the index in the node_name array corresponding to the ground node
...
//If no ground node has been specified, it print's error message and terminates the program.
...
void add_conductance(List *list, complex **A, char **node_name, int num_nodes, char *analysis, FILE *fp1, double w)
{
...
while(p!=NULL)
	...
	if(*p->name=='R' || *p->name=='r')
	   ...
	   A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n1)]+=1/p->value;
		fprintf(fp1, "A[%d][%d]: For resistor %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), p->name, p->n1, 1/p->value, node_name_index(node_name, p->n1), p->n1);
	   A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n2)]+=-1/p->value;
		fprintf(fp1, "A[%d][%d]: For resistor %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name, p->n1, -1/p->value, node_name_index(node_name, p->n2), p->n2);   
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n2)]+=1/p->value;
		fprintf(fp1, "A[%d][%d]: For resistor %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n2), p->name, p->n2, 1/p->value, node_name_index(node_name, p->n2), p->n2);   
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n1)]+=-1/p->value;
		fprintf(fp1, "A[%d][%d]: For resistor %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), p->name, p->n2, -1/p->value, node_name_index(node_name, p->n1), p->n1);   
		}

	if((*p->name=='L' || *p->name=='l') && strcmp(analysis, "ac")==0)
		{
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n1)]+=-I/(w*p->value);
		fprintf(fp1, "A[%d][%d]: For inductor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), p->name, p->n1, -1/(w*p->value), node_name_index(node_name, p->n1), p->n1);
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n2)]+=I/(w*p->value);
		fprintf(fp1, "A[%d][%d]: For inductor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name, p->n1, 1/(w*p->value), node_name_index(node_name, p->n2), p->n2);   
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n2)]+=-I/(w*p->value);
		fprintf(fp1, "A[%d][%d]: For inductor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n2), p->name, p->n2, -1/(w*p->value), node_name_index(node_name, p->n2), p->n2);
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n1)]+=I/(w*p->value);
		fprintf(fp1, "A[%d][%d]: For inductor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), p->name, p->n2, 1/(w*p->value), node_name_index(node_name, p->n1), p->n1);
		}

	if((*p->name=='C' || *p->name=='c') && strcmp(analysis, "ac")==0)
		{
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n1)]+=(p->value)*w*I;
		fprintf(fp1, "A[%d][%d]: For capacitor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), p->name, p->n1, (p->value)*w, node_name_index(node_name, p->n1), p->n1);
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n2)]+=(-p->value)*w*I;
		fprintf(fp1, "A[%d][%d]: For capacitor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name, p->n1, -(p->value)*w, node_name_index(node_name, p->n2), p->n2);
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n2)]+=(p->value)*w*I;
		fprintf(fp1, "A[%d][%d]: For capacitor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n2), p->name, p->n2, (p->value)*w, node_name_index(node_name, p->n2), p->n2);
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n1)]+=(-p->value)*w*I;
		fprintf(fp1, "A[%d][%d]: For capacitor %s, at the node %s, the value %13.5Ej has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), p->name, p->n2, -(p->value)*w, node_name_index(node_name, p->n1), p->n1);
		}

	if(*p->name=='G' || *p->name=='g')
		{
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n3)]+=-p->value;
		fprintf(fp1, "A[%d][%d]: For VCCS %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n3), p->name, p->n1, -p->value, node_name_index(node_name, p->n3), p->n3);
		A[node_name_index(node_name, p->n1)][node_name_index(node_name, p->n4)]+=p->value;
		fprintf(fp1, "A[%d][%d]: For VCCS %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n1), node_name_index(node_name, p->n4), p->name, p->n1, p->value, node_name_index(node_name, p->n4), p->n4);
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n3)]+=p->value;
		fprintf(fp1, "A[%d][%d]: For VCCS %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n3), p->name, p->n2, p->value, node_name_index(node_name, p->n3), p->n3);
		A[node_name_index(node_name, p->n2)][node_name_index(node_name, p->n4)]+=-p->value;
		fprintf(fp1, "A[%d][%d]: For VCCS %s, at the node %s, the value %13.5E has been added corresponding to voltage V[%d] at node %s.\n", node_name_index(node_name, p->n2), node_name_index(node_name, p->n4), p->name, p->n2, -p->value, node_name_index(node_name, p->n4), p->n4);
		}

	if(*p->name=='V' || *p->name=='v')
		{
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n1)]+=1;
		fprintf(fp1, "A[%d][%d]: For voltage source %s,  1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n1), p->name, node_name_index(node_name, p->n1), p->n1); 
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n2)]+=-1;
		fprintf(fp1, "A[%d][%d]: For voltage source %s, -1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n2), p->name, node_name_index(node_name, p->n2), p->n2);
		i++;
		}
	...
	if(*p->name=='E' || *p->name=='e')
		{
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n1)]+=1;
		fprintf(fp1, "A[%d][%d]: For VCVS %s,  1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n1), p->name, node_name_index(node_name, p->n1), p->n1);
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n2)]+=-1;
		fprintf(fp1, "A[%d][%d]: For VCVS %s, -1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n2), p->name, node_name_index(node_name, p->n2), p->n2);
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n3)]+=-p->value;
		fprintf(fp1, "A[%d][%d]: For VCVS %s, %E has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n3), p->name, -p->value, node_name_index(node_name, p->n3), p->n3);
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n4)]+=p->value;
		fprintf(fp1, "A[%d][%d]: For VCVS %s, %E has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n4), p->name, p->value, node_name_index(node_name, p->n4), p->n4);
		i++;
		}
	...
	if(*p->name=='H' || *p->name=='h')
		{
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n1)]+=1;
		fprintf(fp1, "A[%d][%d]: For CCVS %s, 1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n1), p->name, node_name_index(node_name, p->n1), p->n1); 
		A[node_name_index(node_name, "0")+i][node_name_index(node_name, p->n2)]+=-1;
		fprintf(fp1, "A[%d][%d]: For CCVS %s, -1 has been added for voltage V[%d] corresponding at node %s.\n", node_name_index(node_name, "0")+i, node_name_index(node_name, p->n2), p->name, node_name_index(node_name, p->n2), p->n2);
		i++;
		}
...
fprintf(fp1, "A[%d]: The ground node equation for node %s has been written at row %d.\n", node_name_index(node_name, "0"), "0", node_name_index(node_name, "0"));
fprintf(fp1, "\n");
free(p);
}
...
void add_elements_to_B(List *list, complex **B, char **c, char **node_name, int num_nodes, FILE *fp1)
{
...
while(p!=NULL)
   ...
   if(*p->name=='v' || *p->name=='V')
      {
      B[node_name_index(node_name, p->n1)][return_voltage_source_index(c, p->n1, p->n2, list, node_name)]+=1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value  1 has been added corresponding to current I[%d][%d] for voltage source %2s.\n", node_name_index(node_name, p->n1), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n1, node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name);
      B[node_name_index(node_name, p->n2)][return_voltage_source_index(c, p->n1, p->n2, list, node_name)]+=-1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value -1 has been added corresponding to current I[%d][%d] for voltage source %2s.\n", node_name_index(node_name, p->n2), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n2, node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name);
      }
   
   if(*p->name=='e' || *p->name=='E')
      {
      B[node_name_index(node_name, p->n1)][return_voltage_source_index(c, p->n1, p->n2, list, node_name)]+=1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value  1 has been added corresponding to current I[%d][%d] for VCVS %2s.\n", node_name_index(node_name, p->n1), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n1, node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name);
      B[node_name_index(node_name, p->n2)][return_voltage_source_index(c, p->n1, p->n2, list, node_name)]+=-1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value -1 has been added corresponding to current I[%d][%d] for VCVS %2s.\n", node_name_index(node_name, p->n2), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n2, node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->name);
      }
   ...
   if(*p->name=='f' || *p->name=='F')
      {
      B[node_name_index(node_name, p->n1)][return_voltage_source_index_1(c, p->depname, list, node_name)]+=-p->value;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value %E has been added corresponding to current I[%d][%d] for CCCS %2s.\n", node_name_index(node_name, p->n1), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n1, node_name_index(node_name, p->n1), -p->value, node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->depname);
      B[node_name_index(node_name, p->n2)][return_voltage_source_index_1(c, p->depname, list, node_name)]+=p->value;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value %E has been added corresponding to current I[%d][%d] for CCCS %2s.\n", node_name_index(node_name, p->n2), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n2, node_name_index(node_name, p->n2), p->value, node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->depname);
      }

   if(*p->name=='h' || *p->name=='H')
      {
      B[node_name_index(node_name, p->n1)][return_voltage_source_index_1(c, p->depname, list, node_name)]+=1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value 1 has been added corresponding to current I[%d][%d] for CCVS %2s.\n", node_name_index(node_name, p->n1), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n1, node_name_index(node_name, p->n1), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->depname);
      B[node_name_index(node_name, p->n2)][return_voltage_source_index_1(c, p->depname, list, node_name)]+=-1;
      fprintf(fp1, "B[%d][%d]: At node %2s with index %d, a value -1 has been added corresponding to current I[%d][%d] for CCVS %2s.\n", node_name_index(node_name, p->n2), return_voltage_source_index(c, p->n1, p->n2, list, node_name), p->n2, node_name_index(node_name, p->n2), node_name_index(node_name, p->n1), node_name_index(node_name, p->n2), p->depname);
      }
	p=p->next;
	}
for(i=0; i<n_vs; i++)
...
fprintf(fp1, "B[%d]: The ground node equation has been written at row %d.\n", node_name_index(node_name, "0"), node_name_index(node_name, "0"));
fprintf(fp1, "\n");
free(p);
}

//function to return index of the current of a given voltage source
int return_voltage_source_index(char **c, char *s1, char *s2, List *list, char **node_name)
{
...
//function to return index of the current of a given voltage source given the voltage source name
int return_voltage_source_index_1(char **c, char *depname, List *list, char **node_name)
{
Node *p=(Node *)malloc(sizeof(Node));
p=list->head;
while(p!=NULL)
...
void add_sources(List *list, int num_nodes, complex *S, char **node_name, FILE *fp1, char *analysis, char *source, double value)
{
int i, n_vs=num_voltage_source(list);
Node *p=(Node *)malloc(sizeof(Node));
p=list->head;
...
while(p!=NULL)
   ...
   if(*p->name=='I' || *p->name=='i')
      ...
      if(check_ground_node(p->n1, node_name, fp1)==0)
		   ...
		   fprintf(fp1, "S[%d]: At node %2s with index %d, the value %13.5E has been added corresponding to current source %2s.\n", node_name_index(node_name, p->n1), p->n1, node_name_index(node_name, p->n1), p->value, p->name);
		   }
      if(check_ground_node(p->n2, node_name, fp1)==0)
		   {
	      S[node_name_index(node_name, p->n2)]+=-p->value;      
	      fprintf(fp1, "S[%d]: At node %2s with index %d, the value %13.5E has been added corresponding to current source %2s.\n", node_name_index(node_name, p->n2), p->n2, node_name_index(node_name, p->n2), -p->value, p->name);
		   }
      }
   ...
   if(*p->name=='V' || *p->name=='v')
   	...
   	fprintf(fp1, "S[%d]: For voltage source %s, the value at row %d has been added by %E.\n", node_name_index(node_name, "0")+1+i, p->name, node_name_index(node_name, "0")+1+i, value);
   	}
   	else
   	{
   	S[node_name_index(node_name, "0")+1+i]+=p->value;
   	fprintf(fp1, "S[%d]: For voltage source %s, the value at row %d has been added by %E.\n", node_name_index(node_name, "0")+1+i, p->name, node_name_index(node_name, "0")+1+i, p->value);
   	}
   	i++;
   	}
...
for(i=n+1; i<num_nodes+n_vs; i++)
   ...
   for(j=n; j<num_nodes+n_vs+1; j++)
      ...
      //fprintf(fp1, "value ((%E, %E)/(%E, %E))*(%E, %E) = (%E, %E)\n", creal(z), cimag(z), creal(G[n][n]), cimag(G[n][n]), creal(G[n][j]), cimag(G[n][j]), creal((z*G[n][j])/G[n][n]), cimag((z*G[n][j])/G[n][n]));       
...
//change this function (here as well as in the definition) position after all functions are written
//function to deallocate memory allocated to each node
...
if(err==1)
{
fprintf(fp1, "Given source name for analysis has not been defined in the control file.\n");
fprintf(fp1, "Program will terminate.\n\n");
printf("Given source name for analysis has not been defined in the control file.\n");
printf("Program will terminate.\n\n");
exit(8);
}
free(p);
...
new->value=0;
add_node(list, new);
printf("A zero current source %s has been added in place of capacitor %s.\n\n", new->name, p->name);
fprintf(fp1, "A zero current source %s has been added in place of capacitor %s.\n\n", new->name, p->name);
}
...
new->value=0;
add_node(list, new);
printf("A zero voltage source %s has been added in place of inductor %s.\n\n", new->name, p->name);
fprintf(fp1, "A zero voltage source %s has been added in place of inductor %s.\n\n", new->name, p->name);
}
p=p->next;
}
}
...
if(a==3)
fprintf(fp1, "%c[%s][%s]=(%E, %E)\n", c, *(node_name+n1), *(node_name+n2), creal(H[i]), cimag(H[i]));
}
fprintf(fp1, "\n");
}
if(num==0)
...
void print_solutions_to_file(complex *H, char **node_name, int n, FILE *fp1, char *analysis, double value, Node *p)
{
int i, a, n1, n2;
char c;
if(strcmp(analysis, "dc")==0)
...
{
fprintf(fp1, "%15.7E ", value);
fprintf(fp1, "%15.7E ", cabs(H[node_name_index(node_name, p->n1)]-H[node_name_index(node_name, p->n2)]));
fprintf(fp1, "%15.7E ", carg(H[node_name_index(node_name, p->n1)]-H[node_name_index(node_name, p->n2)]));
fprintf(fp1, "\n");
}
}
